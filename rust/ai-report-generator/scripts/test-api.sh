...
IND_BODY=$(curl -s --max-time 10 "$BASE_URL/api/indicators")
...
# 3. Generate report
REPORT_RESPONSE=$(curl -s --max-time 300 -w "\n%{http_code}" -X POST "$BASE_URL/api/reports" \
...
# 4. List reports
LIST_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE_URL/api/reports")
check "GET /api/reports returns 200" "$LIST_STATUS" "200"

LIST_BODY=$(curl -s --max-time 10 "$BASE_URL/api/reports")
...
# 5. Get report by ID
if [[ -n "$REPORT_ID" ]]; then
  GET_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE_URL/api/reports/$REPORT_ID")
  check "GET /api/reports/$REPORT_ID returns 200" "$GET_STATUS" "200"

  GET_BODY=$(curl -s --max-time 10 "$BASE_URL/api/reports/$REPORT_ID")
...
BAD_IND_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/reports" \
  -H "Content-Type: application/json" \
...
BAD_DATE_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/reports" \
  -H "Content-Type: application/json" \
  -d '{"indicators":["UNRATE"],"start_date":"bad","end_date":"2023-12-31"}')
check "POST /api/reports bad date returns 400" "$BAD_DATE_STATUS" "400"

# 8. Report not found
NOT_FOUND_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE_URL/api/reports/00000000-0000-0000-0000-000000000000")
