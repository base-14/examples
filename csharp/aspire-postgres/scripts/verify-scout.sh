...
echo "=== Verify Scout Export (aspire-postgres) ==="
echo ""
...
echo ""
echo "--- Services ---"
SERVICES=$(docker compose logs otel-collector 2>&1 | grep "service.name:" | grep -v "otelcol-contrib" | sort -u || true)
echo "$SERVICES" | grep -q "articles-api" && check "service: articles-api" 0 || check "service: articles-api" 1
echo "$SERVICES" | grep -q "notify-svc" && check "service: notify-svc" 0 || check "service: notify-svc" 1
...
# OTLP IDs: TraceId is 32 hex, SpanId is 16. Filter independently so a
# real SpanId containing the 16-zero substring is not excluded.
CORRELATED_LOGS=$(echo "$COLLECTOR_LOGS" \
    | grep "Trace ID:" \
    | grep -v " -> " \
    | grep -v "Trace ID: $" \
    | grep -Ev "Trace ID: 0{32}( |$)" \
    | head -1)
...
CORRELATED_SPANS=$(echo "$COLLECTOR_LOGS" \
    | grep "Span ID:" \
    | grep -v " -> " \
    | grep -v "Span ID: $" \
    | grep -Ev "Span ID: 0{16}( |$)" \
    | head -1)
...
echo ""
echo "--- Noise Filters ---"
# Span-name lines only. EndpointName on metrics / logs is not subject to
# the traces-pipeline filter/noisy processor.
HEALTH_SPANS=$(echo "$COLLECTOR_LOGS" | grep -E '^[^|]*\| *Name +: .*health' | wc -l | tr -d ' ')
...
echo "Check your Scout dashboard for traces from 'articles-api' and 'notify-svc'."
