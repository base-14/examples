...
# Spring Boot Java 17 + MySQL API Testing Script
#
# NOTE: The delete endpoint in the controller is mapped as
# @DeleteMapping("/users/{id}") under @RequestMapping("/users"),
# making the full path /users/users/{id}.
...
echo -e "${YELLOW}=== Spring Boot Java 17 + MySQL API Tests ===${NC}"
echo ""
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X PUT "$BASE_URL/users/999999" \
    -H "Content-Type: application/json" \
    -d '{"id":999999,"name":"Ghost","address":"Nowhere"}')
check_status "PUT /users/:id (not found)" "404" "$STATUS"

# ------------------------------------------------------------------
# 6. Delete user
# The controller maps @DeleteMapping("/users/{id}") under
# @RequestMapping("/users"), so the full path is /users/users/{id}.
# ------------------------------------------------------------------
echo ""
echo "6. Delete User"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$BASE_URL/users/users/$USER2_ID")
check_status "DELETE /users/users/:id (delete user)" "200" "$STATUS"

STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$BASE_URL/users/users/999999")
check_status "DELETE /users/users/:id (not found)" "404" "$STATUS"
...
curl -s -o /dev/null -X DELETE "$BASE_URL/users/users/$USER1_ID"
