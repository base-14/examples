...
1. Add `io.opentelemetry.instrumentation:opentelemetry-spring-boot-starter` and
   ...
   `opentelemetry-instrumentation-bom` (2.30.0) platform.
...
This example adds JDBC and JPA query spans against PostgreSQL with statement sanitizing, Logback log
records exported over OTLP with trace correlation, Actuator with all Micrometer metrics enabled, and
W3C trace propagation. The full guide is
