...
if [ -n "$TOKEN" ]; then
  ...
  curl -s -X POST "$BASE_URL/api/articles" \
    ...
    -d "{\"title\":\"Scout Test Article $TIMESTAMP\",\"body\":\"Test content for tracing\",\"description\":\"Testing traces\"}" > /dev/null
...
echo "  Traces:"
echo "    - HTTP spans: GET /api/health, POST /api/register, POST /api/articles"
echo "    - Database spans: Hibernate/JDBC queries"
echo "  Metrics:"
