...
# Conversations
CONV_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE_URL/api/conversations")
check "GET /api/conversations returns 200" "$CONV_STATUS" "200"

# Products
PROD_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE_URL/api/products")
check "GET /api/products returns 200" "$PROD_STATUS" "200"

PROD_BODY=$(curl -s --max-time 10 "$BASE_URL/api/products")
...
# Chat - valid message
CHAT_RESPONSE=$(curl -s --max-time 300 -w "\n%{http_code}" -X POST "$BASE_URL/api/chat" \
...
if [[ -n "$CONV_ID" ]]; then
  TURN2_STATUS=$(curl -s --max-time 300 -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/chat" \
    -H "Content-Type: application/json" \
...
# Chat - empty message
BAD_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/chat" \
  -H "Content-Type: application/json" \
  -d '{"message":""}')
check "POST /api/chat empty message returns 400" "$BAD_STATUS" "400"

# Chat - invalid JSON
INVALID_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/chat" \
  -H "Content-Type: application/json" \
  -d 'not json')
check "POST /api/chat invalid JSON returns 400" "$INVALID_STATUS" "400"

# SSE stream endpoint
STREAM_STATUS=$(curl -s --max-time 300 -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/chat/stream" \
  -H "Content-Type: application/json" \
  -d '{"message":"Hello"}')
