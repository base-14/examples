...
TRACES_EXPORTED=$(docker logs slim4-otel-collector 2>&1 | grep -c '"otelcol.signal": "traces"' || true)
METRICS_EXPORTED=$(docker logs slim4-otel-collector 2>&1 | grep -c '"otelcol.signal": "metrics"' || true)
LOGS_EXPORTED=$(docker logs slim4-otel-collector 2>&1 | grep -c '"otelcol.signal": "logs"' || true)
EXPORT_ERRORS=$(docker logs slim4-otel-collector 2>&1 | grep -ciE "(failed to|connection refused|unauthorized)" || true)
