...
# Symfony + MySQL + OpenTelemetry API Testing Script
...
echo "=== Symfony + MySQL API Testing Script ==="
echo "Target: $API_URL"
echo ""
...
# Create an article to trigger notification + trace propagation
...
if [ -z "$TRACE_ID" ]; then
    ...
    # Check collector for spans with matching trace_id
...
# Check for WARN log on 404
if echo "$APP_LOGS" | grep -q '"level_name":"WARNING"'; then
