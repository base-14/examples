...
@pytest.fixture
def analyzer(mock_llm: MagicMock) -> ContentAnalyzer:
    with (
        patch(
        ...
        return ContentAnalyzer(mock_llm)


@patch("content_quality.services.analyzer.generate_structured")
async def test_review_returns_review_result(
    mock_generate: AsyncMock, analyzer: ContentAnalyzer
) -> None:
...
async def test_review_passes_content_type(
    ...
    mock_generate.return_value = ReviewResult(issues=[], summary="Clean", overall_quality="good")
...
async def test_review_passes_system_prompt(
    ...
    mock_generate.return_value = ReviewResult(issues=[], summary="OK", overall_quality="good")
...
async def test_improve_returns_improve_result(
    ...
    mock_generate.return_value = ImproveResult(
...
async def test_improve_passes_content_type(
    ...
    mock_generate.return_value = ImproveResult(suggestions=[], summary="Good")
...
async def test_score_returns_score_result(
    ...
    mock_generate.return_value = ScoreResult(
        score=75,
...
@patch("content_quality.services.analyzer.generate_structured")
async def test_score_passes_endpoint(mock_generate: AsyncMock, analyzer: ContentAnalyzer) -> None:
    mock_generate.return_value = ScoreResult(
        score=50,
...
@patch("content_quality.services.analyzer.evaluation_score")
@patch("content_quality.services.analyzer.trace")
@patch("content_quality.services.analyzer.generate_structured")
async def test_review_emits_evaluation_event(
    mock_generate: AsyncMock,
    mock_trace: MagicMock,
    mock_eval_score: MagicMock,
