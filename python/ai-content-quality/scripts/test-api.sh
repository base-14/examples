...
STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "${BASE_URL}/nonexistent")
check "GET  /nonexistent (404)" "404" "$STATUS"

# ---------------------------------------------------------------------------
# 3. Content type variants - exercises base14.content.type span attribute
# ---------------------------------------------------------------------------
echo ""
echo "$(cyan "=== 3. Content Type Variants ===")"
echo "$(dim "    Each request generates a chat span with base14.content.type attribute")"
echo ""

for CT in marketing technical blog general; do
  BODY=$(curl -s --max-time 600 -X POST "${BASE_URL}/review" \
    -H "Content-Type: application/json" \
...
REVIEW_BODY=$(curl -s --max-time 600 -X POST "${BASE_URL}/review" \
  -H "Content-Type: application/json" \
...
IMPROVE_BODY=$(curl -s --max-time 600 -X POST "${BASE_URL}/improve" \
  -H "Content-Type: application/json" \
...
SCORE_BODY=$(curl -s --max-time 600 -X POST "${BASE_URL}/score" \
  -H "Content-Type: application/json" \
