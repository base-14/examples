...
export async function enqueueArticleCreatedNotification(
  ...
  return tracer.startActiveSpan('job.enqueue.article-created', { kind: SpanKind.PRODUCER }, async (span) => {
    try {
...
export async function enqueueArticleFavoritedNotification(
  ...
  return tracer.startActiveSpan('job.enqueue.article-favorited', { kind: SpanKind.PRODUCER }, async (span) => {
    try {
      span.setAttribute('article.id', payload.articleId);
