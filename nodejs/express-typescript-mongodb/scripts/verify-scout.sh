...
echo "  Express + TypeScript + MongoDB"
echo "========================================"
echo ""
...
REGISTER_RESPONSE=$(curl -s -X POST "$BASE_URL/api/v1/auth/register" \
  ...
  -d "{\"email\":\"$TEST_EMAIL\",\"password\":\"SecurePass123\",\"name\":\"Scout Test\"}")
...
if [ -n "$TOKEN" ]; then
  ...
  curl -s -X POST "$BASE_URL/api/v1/articles" \
    ...
    -d "{\"title\":\"Scout Test Article $TIMESTAMP\",\"content\":\"Testing traces for observability\",\"tags\":[\"test\",\"scout\"]}" > /dev/null
  pass "Article creation request sent"

  curl -s "$BASE_URL/api/v1/articles" > /dev/null
...
echo "  Service: express-mongodb-app"
echo "  Traces:"
echo "    - HTTP spans: GET /api/health, POST /api/v1/auth/register"
echo "    - HTTP spans: POST /api/v1/articles, GET /api/v1/articles"
echo "    - MongoDB query spans (auto-instrumented)"
echo ""
