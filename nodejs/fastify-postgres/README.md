# Fastify 5 + PostgreSQL + OpenTelemetry

A production-ready example demonstrating Fastify 5 REST API with TypeScript,
PostgreSQL, Redis, background jobs (BullMQ), and comprehensive OpenTelemetry
instrumentation for end-to-end observability.

> [Full Documentation](https://docs.base14.io/instrument/apps/auto-instrumentation/fastify)

## How to instrument Fastify with OpenTelemetry

1. Install `@opentelemetry/sdk-node`, `@opentelemetry/auto-instrumentations-node`,
   `@fastify/otel`, `@opentelemetry/exporter-trace-otlp-http`,
...
2. Create `src/telemetry.ts` that builds a `NodeSDK` with `getNodeAutoInstrumentations()` plus
   `new FastifyOtelInstrumentation({ registerOnInitialization: true })` and calls
   `sdk.start()`. Import it as the first line of `src/index.ts` and `src/jobs/worker.ts` with
   `import './telemetry.js'`.
3. Set `OTEL_SERVICE_NAME=fastify-postgres-app` and
...
This example adds Fastify route spans from `@fastify/otel`, pg query spans, BullMQ job trace
propagation into a separate worker service, Pino logs emitted as OTel log records with trace
correlation and a Prometheus `/metrics` endpoint. The full guide is
[Fastify OpenTelemetry Instrumentation](https://docs.base14.io/instrument/apps/auto-instrumentation/fastify/).
...
**Why This Stack**: Demonstrates Fastify 5 with TypeScript for high-performance
...
- **API Layer**: Fastify routes with `/api/*` prefix, CORS, Helmet security
- **Security**: Rate limiting, JWT authentication, bcrypt password hashing
...
cd examples/nodejs/fastify-postgres
```
...
3. Filter by service: `fastify-postgres-app`
...
| `OTEL_SERVICE_NAME`          | Service name in traces | `fastify-postgres-app`      |
...
- `@fastify/otel` plugin for Fastify-specific span enrichment (replaces default auto-instrumentation for Fastify)
- Health and metrics endpoints filtered from tracing
