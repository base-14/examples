...
echo "$SERVICES" | grep -q "trpc-articles" && check "service: trpc-articles" 0 || check "service: trpc-articles" 1
echo "$SERVICES" | grep -q "trpc-notify" && check "service: trpc-notify" 0 || check "service: trpc-notify" 1
...
echo "Check your Scout dashboard for traces from 'trpc-articles' service."
