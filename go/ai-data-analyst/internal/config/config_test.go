...
func TestLoadDefaults(t *testing.T) {
	...
	assert.Equal(t, "qwen3.5:9B", cfg.LLMModelCapable)
	assert.Equal(t, "qwen3.5:9B", cfg.LLMModelFast)
	assert.Equal(t, "ollama", cfg.FallbackProvider)
	assert.Equal(t, "qwen3.5:9B", cfg.FallbackModel)
